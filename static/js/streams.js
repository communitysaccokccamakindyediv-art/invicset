...
const APP_ID = '5fd2190de1bc43fcbe2bbce8ef8c5a07'
...
client.on("volume-indicator", (volumes) => {
    ...
    volumes.forEach((user) => {
        ...
        if (user.level > highestVolume) {
            highestVolume = user.level
            loudestUser = user.uid
        }
    ...
    if (!someoneSpeaking || !loudestUser) return

    const activeCard =
        document.getElementById(`user-container-${loudestUser}`)
